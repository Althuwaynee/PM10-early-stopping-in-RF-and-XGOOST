...
#tiff("XGBoost July ROC.tiff", height = 180, width = 180, units = 'mm', res = 90) #save as tiff
par(mfrow=c(2,2))
roc.Good <- roc(ifelse(predictions$observed=="Good", "Good", "non-Good"), as.numeric(predictions$Good))
roc.VUnheal <- roc(ifelse(predictions$observed== "VUnheal", "VUnheal", "non-VUnheal"), as.numeric(predictions$VUnheal))
roc.Haz <- roc(ifelse(predictions$observed=="Haz", "Haz", "non-Haz"), as.numeric(predictions$Haz))
...
#results= c("Haz AUC" = roc.Haz$auc,"VUnheal AUC" = roc.VUnheal$auc,"Good AUC" = roc.Good$auc)
#print(results)
legend("bottomright",c("Haz","VUnheal","Good"),fill=c("red","#FF9900","green"), cex = 0.7)
#legend("bottomright",c("Haz","VUnheal","Good"),fill=c("red","#FF9900","green"),inset = (0.46), cex = 0.7)
...
# Plot Map
LU_ave<-spplot(r_ave,"Level_ID", main="PM10 ave. concentration prediction: XGBoost July" , 
