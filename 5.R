...
original <-  read.csv("./stat1_July.csv", header = T,stringsAsFactors = FALSE)
original1 <-  read.csv("./stat2_July.csv", header = T,stringsAsFactors = FALSE)
original2 <-  read.csv("./stat3_July.csv", header = T,stringsAsFactors = FALSE)
...
LST = raster("./LST_ave/mean_17_19_LST_july.tif",epsg=32638 )

list.files( "./SAVI_ave",pattern = "tif$", full.names = TRUE)
SAVI <-  raster("./SAVI_ave/mean_july_SAVI.tif" ) 

list.files( "./BU_ave",pattern = "tif$", full.names = TRUE)
BU= raster("./BU_ave/mean_july_BU.tif" ) 

list.files( "./NDMI_ave",pattern = "tif$", full.names = TRUE)
NDMI = raster("./NDMI_ave/mean_july_NDMI.tif" )

list.files( "./NDVI",pattern = "tif$", full.names = TRUE)
NDVI = raster("./NDVI/mean_july_NDVI.tif" )
...
#"band2","band3", "band4","band5","band6","band7"
original= original[c(1,9,11,8,7,10,4,5,6,3,2,12:17)] # re-arrange
head(original)
...
plot(roc.VUnheal, col = "#FF9900", main="All levels prediction of July PM10 data", cex.main = 0.8, cex.lab=0.8)
lines(roc.Haz, col = "red")
lines(roc.Good, col = "green")
legend("bottomright",c("Haz","VUnheal","Good"),fill=c("red","#FF9900","green"), cex = 0.7)
...
roc.Good <- roc(ifelse(predictions$observed=="Good", "Good", "non-Good"), as.numeric(predictions$Good),smoothed = TRUE,ci=TRUE, 
                ...
                print.auc=TRUE, show.thres=TRUE,main="Good level prediction of July PM10 data",cex.main = 0.8, cex.lab=0.8)
...
roc.Haz <- roc(ifelse(predictions$observed=="Haz", "Haz", "non-Haz"), as.numeric(predictions$Haz),smoothed = TRUE,ci=TRUE, 
               ci.alpha=0.9, stratified=FALSE, plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
               print.auc=TRUE, show.thres=TRUE,main="Haz level prediction of July PM10 data",cex.main = 0.8, cex.lab=0.8)
sens.ci <- ci.se(roc.Haz)
plot(sens.ci, type="shape", col="lightblue")
plot(sens.ci, type="bars")

roc.VUnheal <- roc(ifelse(predictions$observed=="VUnheal", "VUnheal", "non-VUnheal"), as.numeric(predictions$VUnheal),smoothed = TRUE,ci=TRUE, 
                   ci.alpha=0.9, stratified=FALSE, plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
                   print.auc=TRUE, show.thres=TRUE,main="VUnheal level prediction of July PM10 data",cex.main = 0.8, cex.lab=0.8)
sens.ci <- ci.se(roc.VUnheal)
plot(sens.ci, type="shape", col="lightblue")
plot(sens.ci, type="bars")


pdf("plot_July_RF.pdf")
...
for (i in 1:16) {
  ...
  proj4string(r_ave_VUnheal)=CRS(projection(Study_area))
  
  # Plot Maps
  print(spplot(r_ave_Haz, main="Haz July RF"))
  writeRaster(r_ave_Haz,filename="Probabiliy_July_RF_Haz.tif", format="GTiff", overwrite=TRUE) 
  
  print(spplot(r_ave_VUnheal, main="VUnheal July RF"))
  writeRaster(r_ave_VUnheal,filename="Probabiliy_July_RF_VUnheal.tif", format="GTiff", overwrite=TRUE) 
  
  print(spplot(r_ave_good, main="GOOD July RF"))
  writeRaster(r_ave_good,filename="Probabiliy_July_RF_GOOD.tif", format="GTiff", overwrite=TRUE) 
  ...
  # Plot Map
  LU_ave<-spplot(r_ave,"Level_ID", main="PM10 ave. concentration prediction: RF July" , 
